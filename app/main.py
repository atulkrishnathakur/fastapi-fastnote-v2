...
async def note_create(new_note: dict):
    note = await notes_services.create_note(
        new_note['title'],
        new_note['content']
    )
    return note
...
async def note_get(note_id: int):
    note = await notes_services.get_node(note_id)
    return note
...
async def note_update(note_id: int, new_note: dict):
    ...
    note = await notes_services.update_note(
        note_id,
        new_title,
        new_content
        )
...
async def note_patch(note_id: int, new_note: dict):
    new_title = new_note.get('title')
    new_content = new_note.get("content")
    note = await notes_services.note_patch(
        note_id,
        new_title,
        new_content
    )
